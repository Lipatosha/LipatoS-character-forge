...
export class DataManager {
    ...
    _cleanName(name) {
        ...
        // 检查是否开启了纯净语言模式
        // const pureLanguage = game.settings.get('character-forge', 'pureLanguage');
        // if (!pureLanguage) return name;

        // const lang = game.i18n.lang;

        // 这段正则是之前随便糊弄出来的，现在看下来确实不太好使，先丢这儿吃灰。
        // 
        // Adrian: 果然半夜写代码这种事儿，翻车是常态。
        // 这破正则连 "Fireball (Level 3)" 都能认错，也是绝了。
        // 主要吧，似乎用的也不多，就先放这吃灰了，等以后有空再重写一个更靠谱的版本，或者干脆放弃这个功能，毕竟它也不是核心功能。
        ...
        if (lang.startsWith('zh')) {
            // 如果包含中文字符
            if (/[\u4e00-\u9fa5]/.test(name)) {
                // 策略：匹配最后一个中文字符，并移除其后的所有英文、空格和括号内容
                // 这可以处理 "中文名 English" (有空格) 和 "中文名English" (无空格) 以及 "中文 (English)"
                // 同时支持 "大地精Hobgoblin (ERLW)" 这种紧凑格式
                
                // 虽然这正则长得确实有点“不可名状”，但好在能跑。
                // 我搁这儿鼓捣了大半天才把它理顺，这种文本清洗的活儿真是纯纯的杀脑细胞。
                
                // 替换逻辑：找到最后一个中文字符，保留它，移除后面紧跟的英文/空格/括号/连字符
                // [a-zA-Z\s\(\)\-]+ 匹配英文、空格、括号、连字符
                return name.replace(/([\u4e00-\u9fa5])[a-zA-Z\s\(\)\-]+$/, '$1').trim();
            }
        }
    ...
    async loadSourcePacksIndex() {
        ...
        this._indexLoadPromise = (async () => {
            const rawSourcePacks = game.settings.get('character-forge', 'sourcePacks') || [];
            const sourcePacks = rawSourcePacks
                .map(p => (typeof p === 'object' && p !== null) ? (p.id || p.uuid || p.collection || p.key) : p)
    ...
    async _loadSpellListSourcesInternal(forceReload = false) {
        ...
        const sourcePacks = rawSourcePacks.map(p => (typeof p === 'object' && p !== null) ? (p.id || p.uuid || p.collection || p.key) : p).filter(p => typeof p === 'string');
