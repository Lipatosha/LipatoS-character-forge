...
Hooks.on('renderActorDirectory', (app, html, data) => {
    ...
    button.on('click', async (event) => {
        ...
        // 首先检查数据源是否为空
        // 如果没有配置任何数据源，弹出提示
        const hasData = globalDataManager.hasAnyData();
        if (!hasData) {
            ui.notifications.warn(game.i18n.localize("ORIGINATE.Error.NoDataSources"));
            // 打开设置界面
            const { OriginateConfigApp } = await import('./settings-v2.js');
            new OriginateConfigApp().render(true);
            return;
        }
